...
// Definition of the initit() function
function init() {
   ...
   // Add an event lisitener for the mouseup event
...
// Definition of the swap puzzle function()
function swapPuzzle(e) {
   // Variable that referecnes which puzzle button was clicked based on its ID
   let puzzleID = e.target.id;
   // Grab the value attribute from the event's target and put it in a variable
...
function setupPuzzle() {
   ...
   document.getElementById("peek").addEventListener("click", function() {
      ...
      }, 500); // end of timeout anon function

   });

   // Check the puzzle solution when user lets go of the mouse button
   document.getElementById("hanjieGrid").addEventListener("mouseup", function() {
      let solved = true;

      // loop every puzzle cell to look at color adn determine if the cell is correct or not
      for(let i = 0; i < puzzleCells.length; i++) {
...
function endBackground() {
   // Loop through puzzleCells array and remove the event listener for every puzzle cell
   for(let i = 0; i < puzzleCells.length; i++) {
